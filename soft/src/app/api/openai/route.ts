...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return new Response(JSON.stringify({error: "Unable to assign IP address. Please try again later."}), {
      status: 500,
      headers: {
  ...
  if (ratelimitresult.remaining === 0) {
    return new Response(JSON.stringify({error: "Rate limit exceeded. Please wait 20 seconds before trying again. You’ve sent too many requests in a short period."}), {
      status: 429,
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    return new Response(JSON.stringify({error: "Daily limit exceeded. You’ve reached your daily quota of requests. Please try again tomorrow."}), {
      status: 429,
      headers: {
        'Content-Type': 'application/json',
      },
