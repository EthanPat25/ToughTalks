...
const ICON = require('../../../public/Restriction.json');

interface airlineprops {
    animate: boolean;
}

export default function RestrictionAnimation({animate}: airlineprops) {   
    const [windowsize, updatewindowsize] = React.useState(window.innerWidth)
    const [size,updatesize] = React.useState(300) 
  ...
  const playerRef = React.useRef<any>(null);
    ...
    <Player size={size} icon={ICON} ref={(instance: typeof Player) => {
                if (instance) {
